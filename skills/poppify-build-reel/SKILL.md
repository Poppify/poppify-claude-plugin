...
- `apply_session_patch({textColor, videoEffect, ...})` — production knobs
- `apply_session_patch({audio:{...}})` — attach music
- `apply_session_patch({visualEdits:[...]})` — pool insert/splice (works only on photo-led pool sessions; use `set_image` instead)
...
// Then attach via apply_session_patch({audio:{...}})
...
Call `get_result({ sessionId })` before confirm — pre-confirm it returns the exact price breakdown (seed total). Then:
...
2. `suggest_live_action({ sessionId, slideIndex })` → pick a motion verb; a `generate_live_motion({dryRun:true, ...})` preview is free (same params, no apiKey needed).
...
## Step 7 — OPTIONAL: consistent frames (`generate_image` consistent-frames mode) + before/after interpolation (`endFrameUri`)
...
**`generate_image` with a reference — a new still of an EXISTING subject (consistent-frames mode; image stage, before motion).** Same cost as a plain generation (5 seeds). Give it a reference subject (`referenceAssetId` or `referenceImageUrl` — a prior `generate_image` result, or an uploaded photo) plus a prompt, and it returns ONE frame of that same subject in the new pose/expression/scene you describe. **Each frame is generated with intent — one deliberate call per frame, never a random batch.** Two consumers:
- **Same subject across slides** → `generate_image` (with the reference) for each slide, then `update_slides({action:"set_image", slideIndex, imageUrl})`, animate normally. This is the consistency path — **no last frame**.
...
generate_image({ referenceImageUrl:<before>, prompt:"same subject, <after state>" })   # the "after"
update_slides({ action:"set_motion_mode", slideIndex, motionMode:"live", liveAction:"<verb>", endFrameUri:<after url> })
generate_live_motion({dryRun:true}) → generate_live_motion    # Veo interpolates before → after
```

Decision shortcut: **same subject across slides → `generate_image` (with the reference) then `set_image`, no last frame. Before/after or morph within one slide → `generate_image` (with the reference) for the "after", then send it as `endFrameUri`.**
...
## Step 8 — OPTIONAL: publish or schedule (linked accounts only)
...
```
publish_post({ apiKey, postId })                                  // no channelIds → returns availableChannels
