...
7. **Optional Live Motion upgrade** (only AFTER the user has seen the cinematic baseline): to animate the subject inside a slide, `search_live_library` first (cache hits free), else `update_slides({action:"set_motion_mode", slideIndex, motionMode:"live", liveAction})` + `generate_live_motion` (10 seeds/clip, Veo 3.1 Lite). Recommend at most one live slide, usually the hook. Never apply it before the baseline render. For before/after transformations (`endFrameUri`), invoke the `poppify-live-motion` skill first — bridged renders need a composition-locked end frame and an authored journey `overridePrompt`.
